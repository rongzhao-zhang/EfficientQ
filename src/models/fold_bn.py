...
def _fold_bn(conv_module, bn_module):
    ...
    w_view = (conv_module.out_channels, 1, 1, 1, 1)  # 3D BN: C_out, C_in, kD, kH, kW
    if bn_module.affine:
...
def reset_bn(module: nn.BatchNorm3d):
...
def is_bn(m):
    return isinstance(m, (nn.BatchNorm2d, nn.BatchNorm3d))


def is_absorbing(m):
    return isinstance(m, (nn.Conv2d, nn.Conv3d, nn.Linear))
