...
def center_crop(tensor, size):
    if len(size) == 3:
        ...
        if d == td and w == tw and h == th:
            return tensor
        # pad to size if smaller
        if w < tw:
            tensor = torch.cat((torch.zeros(tensor.size(0), d, h, (tw-w)//2), tensor, torch.zeros(tensor.size(0), d, h, tw-w-(tw-w)//2)), dim=-1)
            d, h, w = tensor.size()[-3:]
        if h < th:
            tensor = torch.cat((torch.zeros(tensor.size(0), d, (th-h)//2, w), tensor, torch.zeros(tensor.size(0), d, th-h-(th-h)//2, w)), dim=-2)
            d, h, w = tensor.size()[-3:]
        if d < td:
            tensor = torch.cat((torch.zeros(tensor.size(0), (td-d)//2, h, w), tensor, torch.zeros(tensor.size(0), td-d-(td-d)//2, h, w)), dim=-3)
            d, h, w = tensor.size()[-3:]
